...
impl ChunkedVm {
    ...
    fn call_builtin(
        ...
        match builtin {
            ...
            BUILTIN_DISPLAY_REFRESH_MODE => {
                let mode_id = self.pop_sqbc_string_id()?;
                host.draw_refresh_mode(self.index.string(mode_id)?);
            }
    ...
    fn binbook_chapter_entry_record(
        &mut self,
        entry: BinBookChapterEntry<'_>,
    ) -> Result<Value, VmError> {
        let title = self.runtime_string_value(Some(entry.title))?;
        self.runtime_records.alloc(&[
