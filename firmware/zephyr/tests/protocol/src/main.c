...
#endif
	zassert_true(SQ_VM_RUNTIME_WORK_STACK_SIZE <= 18048);
}
...
ZTEST(squidscript_protocol, test_runtime_reuses_transfer_storage_for_init_scratch_and_completion)
{
	...
	zassert_true(runtime_static <= 15264, "runtime_static=%zu", runtime_static);
#endif
}
...
ZTEST(squidscript_protocol, test_resources_report_vm_worker_stack_diagnostics)
{
	...
	zassert_true(response_len <= 834, "resources response_len=%zu", response_len);
