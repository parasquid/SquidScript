...
static void *ble_file_transfer_staging_setup(void)
{
...
static void ble_file_transfer_staging_before(void *fixture)
{
...
static void ble_file_transfer_staging_teardown(void *fixture)
{
	(void)fixture;
	sq_ble_file_transfer_reset_session();
	sq_ble_profile_table_reset();
...
ZTEST_SUITE(ble_file_transfer_staging, NULL, ble_file_transfer_staging_setup, ble_file_transfer_staging_before, NULL,
	    ble_file_transfer_staging_teardown);

ZTEST(ble_file_transfer_staging, test_begin_opens_staging_file)
{
...
ZTEST(ble_file_transfer_staging, test_write_writes_chunks_to_staging_file)
{
	...
	result = sq_ble_file_transfer_test_invoke_write_with_path(staging_path, chunk1,
							    sizeof(chunk1), 0, sizeof(chunk2));
	zassert_equal(result, (int)sizeof(chunk1), "write chunk1 expected %zu, got %d",
		      sizeof(chunk1), result);

	result = sq_ble_file_transfer_test_invoke_write_with_path(staging_path, chunk2,
							    sizeof(chunk2), sizeof(chunk1), 0);
	zassert_equal(result, (int)sizeof(chunk2), "write chunk2 expected %zu, got %d",
...
ZTEST(ble_file_transfer_staging, test_reset_session_unlinks_staging_file)
{
	char staging_path[128] = {0};
	int result;
...
ZTEST(ble_file_transfer_staging, test_second_create_while_busy_returns_obj_locked)
{
	char staging_path_a[128] = {0};
	char staging_path_b[128] = {0};
	int result;

	result = sq_ble_file_transfer_test_invoke_begin_with_name(".sqbc", 4096, staging_path_a,
							      sizeof(staging_path_a));
	zassert_equal(result, 0, "first begin failed: %d", result);

	result = sq_ble_file_transfer_test_invoke_begin_with_name(".sqbc", 4096, staging_path_b,
...
ZTEST(ble_file_transfer_staging, test_write_beyond_declared_size_rejected)
{
	char staging_path[128] = {0};
	uint8_t chunk[16] = {0};
	int result;

	result = sq_ble_file_transfer_test_invoke_begin_with_name(".sqbc", 8, staging_path,
							      sizeof(staging_path));
	zassert_equal(result, 0, "begin failed: %d", result);
	...
	result = sq_ble_file_transfer_test_invoke_write_with_path(staging_path, chunk, 8, 0, 0);
	zassert_equal(result, 8, "expected exact-size write to succeed, got %d", result);
}

ZTEST(ble_file_transfer_staging, test_reset_after_completed_transfer_does_not_error)
{
	char staging_path[128] = {0};
...
ZTEST(ble_file_transfer_staging, test_begin_creates_missing_tmp_dir)
{
	char staging_path[128] = {0};
	int result;
	...
	result = sq_ble_file_transfer_test_invoke_begin_with_name(".sqbc", 16, staging_path,
							      sizeof(staging_path));
	zassert_equal(result, 0, "begin should create the staging dir, got %d", result);
	zassert_true(staging_file_exists(staging_path), "staging file should exist at %s",
		     staging_path);
}
