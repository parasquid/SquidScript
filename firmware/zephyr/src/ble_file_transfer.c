...
LOG_MODULE_REGISTER(squidscript_ble_file_transfer, LOG_LEVEL_INF);
...
const uint8_t sq_ble_file_transfer_adv_uuid[16] = {SQ_XFER_SVC_UUID};
 ...
 * (ble_file_transfer_core.c); this shim only frames opcodes onto its API. All GATT
				  ...
				  const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	if (op == SQ_XFER_OP_BEGIN) {
		...
		if (sq_ble_file_transfer_begin_framed(size, name_len) != 0) {
	...
	if (op == SQ_XFER_OP_NAME) {
		if (sq_ble_file_transfer_feed_name(&bytes[1], (size_t)len - 1u) != 0) {
				  ...
				  const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	result = sq_ble_file_transfer_feed_content(buf, len);
	if (result != 0) {
		sq_ble_file_transfer_abort();
	...
	if (sq_ble_file_transfer_pending_is_complete()) {
...
BT_GATT_SERVICE_DEFINE(sq_ble_file_transfer_svc,
...
/* Attribute layout of sq_ble_file_transfer_svc.attrs:
 *   [0] primary service
 ...
 *   [7] status CCC
 */
#define SQ_XFER_STATUS_VALUE_ATTR (&sq_ble_file_transfer_svc.attrs[6])
...
const uint8_t sq_ble_file_transfer_adv_uuid[16] = {0};
