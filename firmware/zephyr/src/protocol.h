#ifndef SQUIDSCRIPT_PROTOCOL_H
#define SQUIDSCRIPT_PROTOCOL_H

#include <stddef.h>
#include <stdint.h>

#define SQ_PROTOCOL_HEADER_LEN 20u

enum sq_protocol_result {
	SQ_PROTOCOL_OK = 0,
	SQ_PROTOCOL_ERR_TRUNCATED_HEADER = -1,
	SQ_PROTOCOL_ERR_BAD_MAGIC = -2,
	SQ_PROTOCOL_ERR_LENGTH_MISMATCH = -3,
	SQ_PROTOCOL_ERR_PAYLOAD_CRC = -4,
	SQ_PROTOCOL_ERR_TRUNCATED_FIELD = -5,
	SQ_PROTOCOL_ERR_BUFFER_TOO_SMALL = -6,
};

enum sq_frame_kind {
	SQ_FRAME_REQUEST = 1,
	SQ_FRAME_RESPONSE = 2,
	SQ_FRAME_EVENT = 3,
};

enum sq_opcode {
	SQ_OPCODE_HELLO = 1,
	SQ_OPCODE_APP_INSTALL_BEGIN = 16,
	SQ_OPCODE_APP_INSTALL_CHUNK = 17,
	SQ_OPCODE_APP_INSTALL_COMMIT = 18,
	SQ_OPCODE_RESOURCE_INSTALL_BEGIN = 19,
	SQ_OPCODE_RESOURCE_INSTALL_CHUNK = 20,
	SQ_OPCODE_RESOURCE_INSTALL_COMMIT = 21,
	SQ_OPCODE_TEMP_RUN_BEGIN = 24,
	SQ_OPCODE_TEMP_RUN_CHUNK = 25,
	SQ_OPCODE_TEMP_RUN_COMMIT = 26,
	SQ_OPCODE_APP_LAUNCH = 32,
	SQ_OPCODE_APP_LIST = 33,
	SQ_OPCODE_KEY = 48,
	SQ_OPCODE_EVENT_DISPATCH = 49,
	SQ_OPCODE_OUTPUT_GET = 64,
	SQ_OPCODE_STATE_GET = 65,
	SQ_OPCODE_DRAWLOG_GET = 66,
	SQ_OPCODE_TRACE_GET = 67,
	SQ_OPCODE_ERRORS_GET = 68,
	SQ_OPCODE_RESOURCES_GET = 69,
	SQ_OPCODE_LIFECYCLE_GET = 70,
	SQ_OPCODE_STATE_IMPORT = 72,
	SQ_OPCODE_WIFI_PROFILE_SET = 76,
	SQ_OPCODE_RESET = 80,
	SQ_OPCODE_STORAGE_FORMAT = 81,
	SQ_OPCODE_RUNTIME_CAP_GET = 82,
	SQ_OPCODE_RUNTIME_CAP_SET = 83,
	SQ_OPCODE_RUNTIME_CAP_CLEAR = 84,
	SQ_OPCODE_DISPLAY_WINDOW_PROBE = 85,
	SQ_OPCODE_CONTENT_INSTALL_BEGIN = 88,
	SQ_OPCODE_CONTENT_INSTALL_CHUNK = 89,
	SQ_OPCODE_CONTENT_INSTALL_COMMIT = 90,
	SQ_OPCODE_CONTENT_CHECK = 91,
	SQ_OPCODE_DEBUG_LOG_GET = 92,
};

enum sq_status {
	SQ_STATUS_OK = 0,
	SQ_STATUS_ERROR = 1,
	SQ_STATUS_PENDING = 2,
};

struct sq_protocol_frame {
	uint8_t kind;
	uint8_t opcode;
	uint8_t status;
	uint32_t sequence;
	const uint8_t *payload;
	uint32_t payload_len;
	uint32_t payload_crc;
};

struct sq_protocol_request {
	uint8_t opcode;
	uint32_t sequence;
};

uint32_t sq_protocol_crc32(const uint8_t *data, size_t len);
int sq_protocol_decode_frame(const uint8_t *bytes, size_t len, struct sq_protocol_frame *out);
int sq_protocol_decode_request(const uint8_t *bytes, size_t len,
			       struct sq_protocol_request *out);

#endif
