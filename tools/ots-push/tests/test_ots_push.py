...
def test_parse_object_name_valid():
    assert parse_object_name(".sqbc") == ".sqbc"


def test_parse_object_name_rejects_routed_segments():
    with pytest.raises(ValueError):
        parse_object_name("break-reminder/wallpaper/.sqbc")


def test_parse_object_name_empty():
    with pytest.raises(ValueError):
        parse_object_name("")
...
def test_parse_object_name_extension_must_start_with_dot():
    with pytest.raises(ValueError):
        parse_object_name("sqbc")


def test_build_object_name_canonical():
    assert build_object_name() == ".sqbc"
