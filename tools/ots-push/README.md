...
firmware's custom BLE GATT app-transfer service.
...
```sh
python -m ots_push push <device-name-or-address> <source.sqbc>
```

Example:

```sh
python -m ots_push push AA:BB:CC:DD:EE:FF /tmp/installed-app.sqbc
```

The BLE Object Name is `.sqbc`. Firmware delivers the uploaded file to the
foreground app's active `service.ble.start("object-transfer", ...)` profile.
...
```sh
PYTHONPATH=tools/ots-push pytest -q tools/ots-push/tests
```
