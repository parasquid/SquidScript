"""BLE file-transfer client for SquidScript devices (custom GATT transport).

Drives the device's custom GATT file-transfer service over plain GATT writes,
which work cross-platform through bleak (Linux/macOS/Windows) and mirror what a
Web Bluetooth page does. The file name carries only the accepted file
extension; firmware delivers the upload to the foreground app's active profile.

Protocol (matches firmware/zephyr/src/ble_file_transfer.c):
  control char  write: [0x01][size: LE u32][file-name UTF-8]  (BEGIN)
                       [0x03]                                    (ABORT)
...
def parse_file_name(name: str) -> str:
    """Parse an extension-only file name; raise ValueError if malformed."""
    extension = name
    if "/" in extension:
        raise ValueError(f"File name must not contain route segments: {name!r}")
    if not extension:
...
def build_file_name(extension: str = ".sqbc") -> str:
    if not extension.startswith("."):
        raise ValueError(f"Extension must start with '.': {extension!r}")
    return extension
...
def build_begin_command(size: int, name_len: int) -> bytes:
    """Frame the control BEGIN write: opcode + content size + file-name length.
...
async def push_file(
    ...
    *,
    ...
    """Upload a file to the device's custom GATT file-transfer service.
    ...
    if not os.path.isfile(source_path):
        return BleFilePushResult(False, ".sqbc", 0, f"source file not found: {source_path}")
    ...
    if bleak is None:
        try:
            ...
            return BleFilePushResult(False, ".sqbc", 0, "bleak is unavailable")
    if not hasattr(bleak, "BleakScanner") or not hasattr(bleak, "BleakClient"):
        return BleFilePushResult(False, ".sqbc", 0, "bleak API unavailable on this host")
    ...
    return await _push_via_gatt(bleak, device_address, file_name, payload, file_size)


async def _push_via_gatt(
    bleak, device_address: str, file_name: str, payload: bytes, file_size: int
) -> BleFilePushResult:
    extension = parse_file_name(file_name)
    ...
    try:
        ...
        device = None
    if device is None:
        return BleFilePushResult(False, extension, 0, "device not found / no Bluetooth adapter")
    ...
    async with bleak.BleakClient(device) as client:
        ...
        try:
            ...
            if uuids and SVC_UUID not in uuids:
                return BleFilePushResult(False, extension, 0,
                                     f"file-transfer service {SVC_UUID} not found on device")
        except Exception:
            pass

        await client.start_notify(STAT_UUID, on_status)

        # BEGIN declares content size + file-name length (fits the default MTU).
        name_bytes = file_name.encode("utf-8")
        await client.write_gatt_char(CTRL_UUID, build_begin_command(file_size, len(name_bytes)),
                                     response=True)
        # NAME writes carry the file name in MTU-sized pieces.
        ...
        try:
            ...
            return BleFilePushResult(False, extension, sent, "timed out waiting for completion")
        await client.stop_notify(STAT_UUID)

    if status["code"] != STATUS_COMPLETE:
        return BleFilePushResult(False, extension, sent,
                             f"device reported error status {status['code']}")
    return BleFilePushResult(True, extension, file_size)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="ble_file_push")
    ...
    if not result.pushed:
        print(f"OK ble-file-push skipped because {result.skipped_reason or 'unknown'}", file=sys.stdout)
    ...
    print(f"OK ble-file-push uploaded ext={result.extension} bytes={result.bytes_sent}")
    return 0
