...
usage() {
	cat <<'USAGE'
Usage: scripts/zephyr-test-ble-ots-staging.sh [-- <extra twister args>]

Runs the BLE OTS staging-lifecycle ztests through Twister on
native_sim/native/64. The tests exercise obj_created (opens staging
file), obj_write (writes chunks via fs_write + fs_seek), abort
(unlinks + clears in-flight session), reset_session (unlinks), and
the second-create-while-busy OBJ_LOCKED rejection. The tests mount a
...
west twister \
	-T "${ROOT}/firmware/zephyr/tests/ble-ots-staging" \
