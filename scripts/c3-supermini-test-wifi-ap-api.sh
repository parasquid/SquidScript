...
WORK_DIR="${ROOT}/target/hardware-tests/wifi-ap"
...
assert_no_raw_network_identifiers() {
  ...
  if grep -Fq "SquidScript" "${file}"; then
    printf 'Expected %s not to contain the raw AP SSID\n' "${file}" >&2
    printf '%s\n' "--- ${file} ---" >&2
    sed -n '1,200p' "${file}" >&2
    exit 1
  fi
...
assert_file_contains "${apps_out}" "app=wifi-ap-summary"
...
printf '%s\n' 'OK Zephyr Wi-Fi AP SquidScript hardware check passed'
