...
assert_file_contains "${apps_out}" "app=system-resources"

run_capture launch-system-resources cargo run --quiet -p squidc -- app launch system-resources >/dev/null

memory_out="$(wait_for_contains output-memory "output=system memory RAM" \
  "device output" cargo run --quiet -p squidc -- device output)"
assert_file_contains "${memory_out}" "heap"
assert_file_contains "${memory_out}" "free"
assert_file_contains "${memory_out}" "output=system apps Apps"

errors_out="$(run_capture errors cargo run --quiet -p squidc -- device errors)"
assert_file_empty_command "${errors_out}"

printf '%s\n' 'OK Zephyr system resource SquidScript hardware check passed'
