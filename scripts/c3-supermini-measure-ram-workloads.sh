...
reset_heap_max_attribution() {
  local label="$1"
  run_capture "heap-reset-${label}" cargo run --quiet -p squidc -- device resources --reset-heap-max >/dev/null
...
reset_heap_max_attribution display
...
reset_heap_max_attribution system
...
reset_heap_max_attribution wifi-ap-start
...
reset_heap_max_attribution wifi-ap-stop
