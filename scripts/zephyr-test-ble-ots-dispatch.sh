...
usage() {
	cat <<'USAGE'
Usage: scripts/zephyr-test-ble-ots-dispatch.sh [-- <extra twister args>]

Runs the BLE OTS dispatch-handoff ztests through Twister on
native_sim/native/64. The tests verify the producer (obj_write final
...
west twister \
	-T "${ROOT}/firmware/zephyr/tests/ble-ots-dispatch" \
