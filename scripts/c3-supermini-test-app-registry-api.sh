...
WORK_DIR="${ROOT}/target/hardware-tests/app-registry-api"
REGISTRY_APP="${ROOT}/tests/hardware/c3-supermini/app-registry-summary/main.squid"
...
run_capture() {
  ...
  printf 'hardware app registry API: %s\n' "$*" >&2
  "$@" >"${out}" 2>&1
  printf '%s\n' "${out}"
}
...
run_capture install-registry cargo run --quiet -p squidc -- app install "${REGISTRY_APP}" >/dev/null

apps_out="$(run_capture app-list cargo run --quiet -p squidc -- app list)"
assert_file_contains "${apps_out}" "app=app-registry-summary"
...
printf '%s\n' 'OK Zephyr app registry SquidScript hardware check passed'
