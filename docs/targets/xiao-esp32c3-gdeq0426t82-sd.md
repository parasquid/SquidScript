...
| --- | --- | --- | --- | --- |
| `display.epd` | epaper-display | cs=`GPIO3`, dc=`GPIO5`, rst=`GPIO2`, busy=`GPIO4` | firmware-backed-hardware-verified |  |
| `indicator.default` | not-present |  | metadata-confirmed | The canonical XIAO ESP32-C3 + DESPI-C02 e-paper setup has no firmware-controllable onboard LED wired to a numbered GPIO. The visible board state is the GDEQ0426T82 e-paper display, not an LED. SquidScript visible-state checks must use the e-paper display, serial output, or e-paper drawlog evidence; LED-observation-based tests such as scripts/c3-supermini-test-blinky.sh and scripts/c3-supermini-test-blink.sh are Super-Mini-only and will not work on the XIAO. |
